...
export function generateMetadata(): Metadata {
  if (!isPlatformControlPlane()) return {}
  return {
...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  const controlPlane = isPlatformControlPlane()
          ...
          <section className="hidden rounded-[32px] border border-white/60 bg-[#0f2d55] p-10 text-white shadow-[0_24px_70px_rgba(15,45,85,0.22)] lg:flex lg:flex-col lg:justify-between">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h1 className="max-w-xl text-4xl font-semibold tracking-[-0.05em]">
                  {controlPlane
                    ? 'Operate every customer deployment from one private vendor workspace.'
                ...
                <p className="max-w-xl text-base leading-7 text-slate-200">
                  {controlPlane
                    ? 'Track customers, isolate installations, manage exact releases, and monitor health without entering a clinic’s patient system.'
                    : 'Appointments, patient history, treatment plans, ETB billing, and multilingual communication stay in one workspace built for Habesha clinics.'}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-3xl border border-white/10 bg-white/8 p-5">
                <p className="text-2xl font-semibold">{controlPlane ? '1 owner' : 'ETB'}</p>
                <p className="mt-2 text-sm text-slate-200">
                  {controlPlane
                    ? 'Private freelancer access'
                    : 'Finance-ready pricing and collections'}
                </p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/8 p-5">
                <p className="text-2xl font-semibold">{controlPlane ? 'Isolated' : 'EN / አማ'}</p>
                <p className="mt-2 text-sm text-slate-200">
                  {controlPlane
                    ? 'Separate customer databases'
                    : 'English and Amharic-ready patient flows'}
                </p>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/8 p-5">
                ...
                <p className="mt-2 text-sm text-slate-200">
                  {controlPlane
                    ? 'Products, releases, and health'
                    : 'Reception, clinical care, and billing aligned'}
                </p>
              </div>
            </div>
          </section>
