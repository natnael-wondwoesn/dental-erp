...
export function renderPrescriptionPaperHtml(
  ...
  <div class="toolbar"><button onclick="window.print()">Print prescription</button></div><main class="paper"><header class="header">${logo}<div><div class="am">ሰኒ ስማይል ስፔሻሊቲ የጥርስ ክሊኒክ</div><div class="clinic">${e(clinic.name || 'Sunny Smile Speciality Clinic')}</div></div><div class="contact">${e([clinic.address, clinic.city].filter(Boolean).join(', '))}<br>${e(clinic.phone || '')}<br>${e(clinic.email || '')}</div></header>
  ...
  <footer class="footer">Issued by ${e(clinic.name || 'Sunny Smile Speciality Clinic')} · ${e(data.prescriptionNo)}</footer></main></body></html>`
}
